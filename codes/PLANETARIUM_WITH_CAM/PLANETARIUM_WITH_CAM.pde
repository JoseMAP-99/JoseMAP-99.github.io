 ...
 * Planetario con Camera -- mintiendo en medidas para mejor ajuste
 */
...
//--------Variables de estado----------//
boolean btnHelp, newDraw, naveCamara, controlsNave;
...
void mousePressed() {    
  ...
  if (btnHelp) {
    if (mouseX > help.getXLeft('A') && mouseX < help.getXRight('A') && mouseY > help.getYUp('A') && mouseY < help.getYDown('A')) {
      controlsNave = !controlsNave;  
    }
...
void moveShape() {  
  ...
  if (keys[9] && naveCamara) {
    nave.updateX(1);
  }  
  if (keys[10] && naveCamara) {
    nave.updateX(-1);
  }
}
